...
import fenrir.util.vornoi.event.VertexEvent;
...
public class LeafBeachNode extends BeachNode {
    ...
    public InsertionResult insertArc(Point newSite) {
        ...
        Optional<LeafBeachNode> thisLeafOpt= Optional.of(this); 
        return new InsertionResult(thisLeafOpt, newLeaf);
    }
    ...
    private static Optional<VertexEvent> buildEvent(LeafBeachNode center) {
    	...
    	Optional<LeafBeachNode> centerNeighbor = center.getLeftNeighbor(); 
    	if(!centerNeighbor.isPresent()) {return Optional.empty();}
    	
    	
        return center.getLeftNeighbor().flatMap(l -> center.getRightNeighbor().flatMap(r -> VertexEvent.build(l, center, r))); //VertexEvent.build(l, center, r)));
    }
